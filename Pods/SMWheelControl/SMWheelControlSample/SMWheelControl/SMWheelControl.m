...
static const CGFloat kMaxVelocity = 100.0;
static const CGFloat kSMDecelerationMultiplier = 0.98;
...
- (void)drawWheel
{
    ...
    UIImageView *image = [[UIImageView alloc]initWithFrame:self.sliceContainer.frame];
    image.clipsToBounds = YES;
    [image setImage:[UIImage imageNamed:@"randomizer_wheel2"]];
    [image setContentMode:UIViewContentModeScaleAspectFit];
   // [image setBackgroundColor:[UIColor greenColor]];
    [self.sliceContainer addSubview:image];
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    if (_status == SMWheelControlStatusIdle && touch.tapCount > 0 && _detectingTap) {
        ...
        if(_status!= SMWheelControlStatusDecelerating)
...
- (void)snapToNearestSlice
{
    ...
    // To check if same slice is getting selected once again.
//    NSString *strClosestSlice = [NSString stringWithFormat:@"%d",closestSlice];
//    if ([arrSlices containsObject:strClosestSlice]) {
//        closestSlice = closestSlice + 1;
//        NSString *strNewClosestSlice = [NSString stringWithFormat:@"%d",closestSlice];
//        [arrSlices addObject:strNewClosestSlice];
//    } else {
//        [arrSlices addObject:strClosestSlice];
//    }
